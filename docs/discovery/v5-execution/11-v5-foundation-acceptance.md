...
"v5" is the name of the architecture phase, not a requirement for a matching
EasyStarter release number.
...
| URL Next | third pure-directory adapter | product-owned seam for existing public reads, page-family cutover, and response-level Worker verification; main `11f64909c76b36e3a1460a25f836e935b88c92fd` | yes, but promotes no additional Foundation capability |
...
Explorer UI remains product-local. Prompt Dir, AIBranding, and URL Next have
...
three-product evidence of substantially identical behavior and props to move
upstream.
...
- URL Next product features, data changes, custom-domain cutover, or a shared
...
2. Treat AIBranding's remaining production-cutover work as product work, not
   Foundation work.
3. Use the [directory adapter golden path](../../golden-paths/directory.md)
   for future product-owned adapters; it does not authorize a directory kit.
4. Consider a UI extraction only after a separate three-way props and behavior
   audit, not merely because three products have a directory page.
