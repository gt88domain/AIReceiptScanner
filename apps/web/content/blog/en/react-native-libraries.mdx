...
thumbnail: "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=900&auto=format&fit=crop&q=60"
author: "dillion"
...
export default function App() {
  return (
    <>
      ...
      />
      <Button title="Hello World" onPress={() => console.log("Button pressed")} />
    </>
  );
}
```
...
function Profile({ userId }) {
  const { data, isLoading, error } = useQuery(["profile", userId], () => fetchProfile(userId));
