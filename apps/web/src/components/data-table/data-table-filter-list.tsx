...
import { formatDateWithOptions } from "@repo/shared";
...
interface DataTableFilterListProps<TData>
  ...
  debounceMs?: number;
  disabled?: boolean;
  /** Filters state (required) */
  filters: ExtendedColumnFilter<TData>[];
  /** Filters change handler (required) */
  onFiltersChange: (
    filters: ExtendedColumnFilter<TData>[] | null | ((prev: ExtendedColumnFilter<TData>[]) => ExtendedColumnFilter<TData>[])
  ) => void;
  /** Join operator state (required) */
  joinOperator: JoinOperator;
  /** Join operator change handler (required) */
...
export function DataTableFilterList<TData>({
  ...
  const debouncedSetFilters = useDebouncedCallback(onFiltersChange, debounceMs);
    ...
    case "dateRange": {
      ...
      const displayValue =
        filter.operator === "isBetween" && dateValue.length === 2 && !isSameDate
          ? `${formatDateWithOptions(startDate, { month: "short" })} - ${formatDateWithOptions(endDate, { month: "short" })}`
          : startDate
            ? formatDateWithOptions(startDate, { month: "short" })
            : "Pick a date";
