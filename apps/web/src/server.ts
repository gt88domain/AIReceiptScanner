...
function applyResponseHeaders(response: Response, pathname: string, serverUrl: string): Response {
  const nextResponse = applySecurityHeaders(response, serverUrl);
...
export default {
  async fetch(req: Request, env: CloudflareBindings): Promise<Response> {
    ...
    if (localeResult.redirect) {
      return applyResponseHeaders(localeResult.redirect, pathname, env.VITE_SERVER_URL);
    }
    ...
    return applyResponseHeaders(localeResponse, pathname, env.VITE_SERVER_URL);
  },
};
